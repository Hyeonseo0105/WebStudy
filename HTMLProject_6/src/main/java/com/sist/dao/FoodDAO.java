...
import javax.sql.*;     // 데이터 베이스 정보 => DataSource
import javax.naming.*;  // 문자열로 객체를 찾는 경우 => Context

// 이름으로 찾기
/*
 *    lookup() => getBean() (String) => 객체찾기
 *                  => 클래스 관리자 (생성 ~ 소멸)
 *                  => 자바기반 => Kotlin
 *                selectList() => SQL
 *                  => SQL 관리자 ==> Mapper
 ...
 *                    => 탐색기를 연다
 *                  Context c=(Context)init.lookup("java://com/env");
 *                  DataSource ds=(DataSource)c.lookup("jdbc/oracle");
 *                  Connection conn=ds.getConnection()
 *          2. conn 사용
 *          3. conn 사용 후에 반환 => POOL영역에 반환
