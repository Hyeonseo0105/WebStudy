...
/*
 *    서블릿 => 장점은 보완 => .java (X) => .class
 *             단점 : HTML작성이 복잡하다(문자열)
 *                     => 수정시마다 컴파일해야 된다
 *          => HTML을 사용하지 않는다
 *          => 자바와 HTML을 연결
 *          => JSP + servlet
 *    JSP => 사용이 편리 , 보안 취약 => .jsp(그대로)
 *        => 컴파일 없이 실행 => 스크립트
 *    M V C
 *      | - Controller - Servlet
 *      |
...
public class FoodListServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(cookies!=null)
		{
			for(int i=cookies.length-1;i>=0;i--)  // 최근방문 기록을 앞으로
			{
				if(cookies[i].getName().startsWith("food"))   // 키(food_+fno)를 읽는다
				{
		...
		if(cList.size()!=0)
		{
			for(FoodVO vo:cList)
			{
				...
				out.write("<img src="+vo.getPoster()+" style=\"width:120px;height:100px;margin:10px 0 0 5px\">");
				                                                                      //100px\">&nbsp;");
				                                                                      //margin:10px  0     0     5px
