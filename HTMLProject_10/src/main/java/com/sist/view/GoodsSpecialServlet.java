...
public class GoodsSpecialServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		{
			out.write("<li><a href=MainServlet?page="+(startPage-1)+"&mode=2>&lt;</a></li>");
			// 11page ==> 이전 ==> 10
		}		
		for(int i=startPage;i<=endPage;i++)
		{
			out.write("<li "+(i==curpage?"class=active":"")+"><a href=MainServlet?page="+i+"&mode=2>"+i+"</a></li>");
		}	
		if(endPage<totalpage)  // 10 20 30..
		{
			out.write("<li><a href=MainServlet?page="+(endPage+1)+"&mode=2>&gt;</a></li>");
		}		
		out.write("</ul>");
		out.write("</div>");
		out.write("</div>");
