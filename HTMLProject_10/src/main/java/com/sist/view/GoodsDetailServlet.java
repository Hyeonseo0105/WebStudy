...
//				out.write("<a href=ReplyDeleteServlet?rno="+rvo.getRno()+"&typeno="+type+"&no="+no+" class=\"btn btn-xs btn-info\">삭제</a>");
//			}
//			out.write("</td>");
//			out.write("</tr>");
//			
...
//		out.write("</body>");
//		out.write("</html>");
//		// => head, style , script => 닫기를 하지 않는 경우에는 흰색이다.
//	}
//
//	@Override
...
public class GoodsDetailServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		for(ReplyVO rvo:list)
		{
			...
			if(rvo.getId().equals(id))
			{
				out.write("<span href=# class=\"btn btn-xs btn-success ups\" data-no="+rvo.getRno()+">수정</span>&nbsp;");
			...
			out.write("<input type=hidden name=gno value="+no+">");
			out.write("<input type=hidden name=typeno value="+type+">");
			out.write("<textarea name=msg rows=4 cols=20 style=\"float:left\">"+rvo.getMsg()+"</textarea>");
