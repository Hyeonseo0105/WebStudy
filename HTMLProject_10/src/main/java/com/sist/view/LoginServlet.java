...
// 아이디가 저장되고 메인페이지로 이동
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		out.write("<!DOCTYPE html>");   // html 5버전 쓸려면 꼭 쓰기
		out.write("<html>");
		out.write("<head>");
		out.write("<link rel=\"stylesheet\" href=\"https://maxcdn.bootstrapcdn.com/bootstrap/3.4.1/css/bootstrap.min.css\">");
		out.write("<style>");
		out.write(".container{margin-top:50px}");
		out.write(".row{margin:0px auto;width:300px}");     // 가운데 정렬
		...
		out.write("<td width:80%>");
		out.write("<input type=password name=pwd size=15 required>");   // required : 팝업창
		out.write("</td>");
		out.write("</tr>");
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(vo.getMsg().equals("OK"))
		{
			...
			session.setAttribute("name", vo.getName());    // 저장하는 과정
			// Map방식 => 키를 중복하지 않는다
		...
		{
			out.write("<script>");
			out.write("alert(\"아이디가 존재하지 않습니다\");");    //alert : 팝업창
		...
		{
			out.write("<script>");
			out.write("alert(\"비밀번호가 틀립니다\");");    //alert : 팝업창
			out.write("history.back();");
			out.write("</script>");
		}
	}
